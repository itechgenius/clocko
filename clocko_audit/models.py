...
from Clocko_audit.methods import remove_duplicate_history
...
class AuditTag(models.Model):
    ...
    class Meta:
        ...
        app_label = "Clocko_audit"


class ClockoAuditInfo(models.Model):
    """
    ClockoAuditInfo model to store additional info
    """
...
class ClockoAuditLog(HistoricalRecords):
    """
    Model to store additional information for historical records.
    """

    # def __init__(self, *args, bases=None, **kwargs):
    #     super(ClockoAuditLog, self).__init__(*args, **kwargs)
    #     self.is_Clocko_audit_log = True
...
def post_create_Clocko_audit_log(sender, instance, *_args, **kwargs):
    ...
    try:
        ...
        if isinstance(history_instance, ClockoAuditLog):
