...
class KeyResult(ClockoModel):
    ...
    history = ClockoAuditLog(bases=[ClockoAuditInfo])
...
class EmployeeObjective(ClockoModel):
    ...
    history = ClockoAuditLog(bases=[ClockoAuditInfo], related_name="history_set")
    archive = models.BooleanField(default=False)
...
class Comment(models.Model):
    ...
    history = ClockoAuditLog(excluded_fields=["comment"], bases=[ClockoAuditInfo])
    objects = ClockoCompanyManager(
...
class QuestionTemplate(ClockoModel):
...
class Question(ClockoModel):
    ...
    objects = ClockoCompanyManager("template_id__company_id")

    def __str__(self):
        return self.question


class QuestionOptions(ClockoModel):
    ...
    objects = ClockoCompanyManager("question_id__template_id__company_id")


class Feedback(ClockoModel):
...
class EmployeeBonusPoint(ClockoModel):
    employee_id = models.ForeignKey(
        Employee,
        on_delete=models.DO_NOTHING,
