...
from Clocko import Clocko_apps
...
from Clocko.Clocko_settings import (
    APPS,
    DB_INIT_PASSWORD,
...
from Clocko.methods import get_Clocko_model_class, remove_dynamic_url
from Clocko_audit.forms import HistoryTrackingFieldsForm
from Clocko_audit.models import AccountBlockUnblock, AuditTag, HistoryTrackingFields
...
def initialize_database(request):
    ...
    if initialize_database_condition():
        ...
        return render(request, "initialize_database/Clocko_user.html")
    else:
        return redirect("/")
...
def initialize_database_user(request):
    ...
    if request.method == "POST":
        ...
        if password != confirm_password:
            return render(request, "initialize_database/Clocko_user_signup.html")
        ...
        return render(
            request,
            "initialize_database/Clocko_company.html",
...
def initialize_database_company(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(
                request,
                "initialize_database/Clocko_department.html",
                {"form": DepartmentForm(initial={"company_id": company})},
            )
    return render(request, "initialize_database/Clocko_company.html", {"form": form})
...
def initialize_database_department(request):
    ...
    return render(
        request,
        "initialize_database/Clocko_department_form.html",
...
def initialize_database_job_position(request):
    ...
    if request.method == "POST":
        ...
        return render(
            request,
            "initialize_database/Clocko_job_position_form.html",
            {
                "form": form,
    ...
    return render(
        request,
        "initialize_database/Clocko_job_position.html",
...
class ClockoPasswordResetView(PasswordResetView):
    """
    Clocko View for Reset Password
    """
...
class EmployeePasswordResetView(PasswordResetView):
    """
    Clocko View for Employee Reset Password
    """
...
def two_factor_auth(request):
    ...
    if not Clocko_apps.TWO_FACTORS_AUTHENTICATION:
        return redirect("/")
...
def object_delete(request, obj_id, **kwargs):
    ...
    if apps.is_installed("pms") and redirect_path == "/pms/filter-key-result/":
        KeyResult = get_Clocko_model_class(app_label="pms", model="keyresult")
...
def mail_server_test_email(request):
    ...
    white_labelling = getattr(Clocko_apps, "WHITE_LABELLING", False)
    image_path = path.join(settings.STATIC_ROOT, "images/ui/Clocko-logo.png")
    company_name = "Clocko"
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            subject = _("Test mail from Clocko")
...
@login_required
@hx_request_required
@permission_required("base.change_Clockomailtemplate")
def view_mail_template(request, obj_id):
    """
    This method is used to display the template/form to edit
    """
    template = ClockoMailTemplate.objects.get(id=obj_id)
...
@login_required
@hx_request_required
@permission_required("base.add_Clockomailtemplate")
...
@login_required
@permission_required("base.delete_Clockomailtemplate")
def delete_mail_templates(request):
    ids = request.GET.getlist("ids")
    result = ClockoMailTemplate.objects.filter(id__in=ids).delete()
...
def employee_shift_view(request):
    ...
    if apps.is_installed("attendance"):
        GraceTime = get_Clocko_model_class(app_label="attendance", model="gracetime")
...
def general_settings(request):
    ...
    if apps.is_installed("payroll"):
        PayrollSettings = get_Clocko_model_class(
            app_label="payroll", model="payrollsettings"
        )
        EncashmentGeneralSettings = get_Clocko_model_class(
...
@login_required
@permission_required("Clocko_audit.change_accountblockunblock")
...
@login_required
@permission_required("Clocko_audit.view_audittag")
...
@login_required
@hx_request_required
@permission_required("Clocko_audit.add_audittag")
...
@login_required
@hx_request_required
@permission_required("Clocko_audit.change_audittag")
...
@login_required
def get_Clocko_installed_apps(request):
